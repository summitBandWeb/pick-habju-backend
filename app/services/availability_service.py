...
class AvailabilityService:
    ...
    async def check_availability(self, request: AvailabilityRequest) -> AvailabilityResponse:
        ...
        for res in processed_results:
            ...
            if res.available is True or res.available == "unknown":
                ...
                bid = res.room_detail.business_id
                if bid not in branch_summary:
                    branch_summary[bid] = BranchStats(
                        min_price=res.room_detail.pricePerHour,
                        available_count=1,
                        lat=res.room_detail.lat,
                        lng=res.room_detail.lng
                    )
                else:
                    stats = branch_summary[bid]
                    stats.available_count += 1
                    if res.room_detail.pricePerHour < stats.min_price:
                        stats.min_price = res.room_detail.pricePerHour
    ...
    def _apply_policies(
        ...
        for res in results:
            ...
            if booking_duration_hours == 1 and not res.room_detail.canReserveOneHour:
            ...
            # 2. 당일 예약 제한
            if request.date == today and res.room_detail.requiresCallOnSameDay:
                policy_warnings.append(PolicyWarning(
            ...
            if res.available is True:
                try:
                    ...
                    price = self.pricing_service.calculate_total_price(
                        base_price=res.room_detail.pricePerHour,
                        price_config=res.room_detail.priceConfig or [],
                        base_capacity=res.room_detail.baseCapacity,
                        extra_charge=res.room_detail.extraCharge,
                        start_dt=start_dt,
                        end_dt=end_dt,
                    ...
                    logger.warning(f"Price calculation failed for {res.room_detail.name}: {e}")
