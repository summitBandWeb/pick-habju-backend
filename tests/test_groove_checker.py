...
def sample_groove_rooms():
    """테스트를 위한 그루브 연습실 RoomDetail 샘플 목록을 제공합니다.

    Rationale:
        기존에는 get_rooms_by_criteria()로 실제 DB에서 데이터를 가져왔으나,
        DB 상태에 따라 테스트가 깨지는 문제가 있었음 (IndexError).
        외부 의존성을 제거하고 하드코딩된 Mock 데이터를 사용하여
