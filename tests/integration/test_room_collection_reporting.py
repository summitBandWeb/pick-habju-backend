...
@pytest.mark.asyncio
async def test_collect_priority_areas_reports_skipped_domain_and_no_room_cases():
    with patch("app.services.room_collection_service.NaverMapCrawler") as mock_crawler_cls, \
         patch("app.services.room_collection_service.NaverRoomFetcher"), \
         patch("app.services.room_collection_service.RoomParserService"), \
         patch("app.services.room_collection_service.get_supabase_client"):
        ...
        async def collect_by_id_side_effect(business_id: str):
            if business_id == "biz2":
                return {"status": "skipped_non_rehearsal", "reason": "no_rehearsal_keyword_match"}
            if business_id == "biz3":
                return {"status": "skipped_no_rooms", "reason": "biz_items_empty"}
            return {"status": "collected"}

        service.collect_by_id = AsyncMock(side_effect=collect_by_id_side_effect)
        ...
        assert result["skipped_non_rehearsal"] == 1
        assert result["skipped_no_rooms"] == 1
