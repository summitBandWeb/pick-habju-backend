...
| 시간대/요일 운영 태그 제거 | `[평일]`, `(주말)`, `심야 한정` 등 | `[블랙룸 심야 한정 운영] 블랙룸` -> `블랙룸` |
...
### 규칙
- **가격이 0이거나 없는 룸은 수집하지 않는다** (`_has_reservation_metadata`에서 제외)
- 개인연습실 등 시간당 5,000원 미만 룸은 합주실이 아닐 가능성이 높으므로 주의
...
| `requires_contact_on_sameday` | 당일 예약 시 연락 필요 | true면 "당일 예약은 전화 문의" 경고 |

---

## 6. 이미지 (`image_urls`)

- JSONB 배열. 빈 배열 `[]` 허용, NULL은 빈 배열로 변환
...
| 합주실 키워드 미매칭 | 합주실이 아님 |
| 가격 정보 없음 (0원 포함) | 유효한 예약 정보 제공 불가 |
...
-- 2) 가격 0인 룸
SELECT count(*) FROM room WHERE price_per_hour <= 0 OR price_per_hour IS NULL;
...
WHERE recommend_capacity > max_capacity
   OR max_capacity > 50
   OR max_capacity <= 0;
```
